...
void loadPCD (NpyArray vert_arr, NpyArray sync_arr, Mat confimg, PointCloud& pointcloud, bool box, bool planecut)
{
  ...
  //cout<<pointcloud.points.size()<<" "<<confimg.cols<<endl;
...
int main (int argc, char** argv)
{
   ...
   vertfile<<data_folder<<argv[1]<<"/"<<argv[2]<<"/"<<"vert-"<<setfill('0')<< setw(3)<<k<<".npy";
   vert_arr = npy_load(vertfile.str());
   //cout<<vertfile.str()<<endl;

   ostringstream syncfile;
   syncfile<<data_folder<<argv[1]<<"/"<<argv[2]<<"/"<<"sync-"<<setfill('0')<< setw(3)<<k<<".npy";
   sync_arr = npy_load(syncfile.str());
   //cout<<syncfile.str()<<endl;
   
   ostringstream conffile;
   conffile<<data_folder<<argv[1]<<"/"<<argv[2]<<"/"<<"confidence-"<<setfill('0')<< setw(3)<<k<<".png";
   ...
   pcdfile_raw << svg <<  argv[3] << "/raw/" << setfill('0') << setw(3) << k <<".pcd";
